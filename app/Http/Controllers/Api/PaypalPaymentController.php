...
class PaypalPaymentController extends Controller
{
    ...
    public function user_subscriptions_payment_by_paypal_direct(Request $request) {
        ...
        try {
            ...
            if($user_pay_amount > 0) {
                ...
                $data['return_url'] = route('user.ppv_payment.success');

                $data['cancel_url'] = route('user.ppv_payment.cancel');
     ...
     */
    public function ppv_payment_success(Request $request)
    {   
        ...
        if (in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            ...
            $response_array = [ 'success' => true, 'message' => tr('payment_failure'), 'amount' => $user_subscription_payment->amount];
