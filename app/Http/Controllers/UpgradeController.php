...
class UpgradeController extends Controller
{
    ...
    public function free_subscriptions_clear(Request $request) {
        ...
        $user_subscriptions = \App\UserSubscription::where('monthly_amount', '>', 0)->orWhere('yearly_amount', '>', 0)->get();

        foreach ($user_subscriptions as $key => $user_subscription) {
        
            $change_expiry_user_ids = \App\UserSubscriptionPayment::where('user_subscription_id', 0)->where('to_user_id', $user_subscription->user_id)->pluck('from_user_id')->implode(',') ?? "";
